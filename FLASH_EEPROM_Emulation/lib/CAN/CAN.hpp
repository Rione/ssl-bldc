...
class CAN {
  public:
    ...
    CAN(CAN_HandleTypeDef *_hcan, uint32_t _myId)
    ...
    void init() {
        ...
        if (HAL_CAN_ConfigFilter(&hcan1, &filter) != HAL_OK) Error_Handler();                             // フィルター設定
        if (HAL_CAN_ActivateNotification(&hcan1, CAN_IT_RX_FIFO0_MSG_PENDING) != HAL_OK) Error_Handler(); // 受信割り込み設定
        printf("- CAN init\n");
    }
