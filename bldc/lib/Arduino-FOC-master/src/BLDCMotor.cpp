...
int BLDCMotor::alignSensor() {
    ...
    if (!_isset(zero_electric_angle)) {
        ...
        monitor_port->println(F("MOT: Skip offset calib."));
    return exit_flag;
}
...
int BLDCMotor::absoluteZeroSearch() {
    ...
    if (monitor_port) {
        if (sensor->needsSearch())
            monitor_port->println(F("MOT: Error: Not found!"));
        else
            monitor_port->println(F("MOT: Success!"));
    }
...
void BLDCMotor::loopFOC() {
    ...
    case TorqueControlType::voltage:
        ...
        monitor_port->print(F("el:"));
        monitor_port->print(electrical_angle);
        monitor_port->print(F("\tq:"));
        monitor_port->print(voltage.q);
        monitor_port->print(F("\td:"));
        monitor_port->println(voltage.d);
        break;
    ...
    case MotionControlType::torque:
        if (torque_controller == TorqueControlType::voltage) { // if voltage torque control
            if (!_isset(phase_resistance))
                ...
                voltage.q = target * phase_resistance;
            voltage.d = 0;
        } else {
